...
export function MatchesProvider({ children }: { children: React.ReactNode }) {
  const [data, setData] = useState<{ today: MatchListResponse | null; tomorrow: MatchListResponse | null }>({
    today: null,
    tomorrow: null,
  })
  ...
  const recordMatchAssets = useCallback((detail: MatchDetail | null) => {
  ...
  const getMatchAssets = useCallback((matchId: number | string) => {
    ...
    async (matchId: number | string) => {
      ...
      if (!detailRequests.current[numeric]) {
        detailRequests.current[numeric] = fetchMatchDetail(numeric)
          ...
          .catch((error) => {
            console.warn('match detail fetch failed', numeric, error)
            return null
          })
          .finally(() => {
  ...
  const getOrFetchMatchAssets = useCallback(
    async (matchId: number | string) => {
      ...
      if (!assetRequests.current[numeric]) {
        assetRequests.current[numeric] = fetchMatchDetail(numeric)
          ...
          .catch((error) => {
            console.warn('match asset fetch failed', numeric, error)
            return null
          })
          .finally(() => {
    ...
    [matchAssets, matchDetails, recordMatchAssets, recordMatchDetail],
  )
  ...
  const requestView = useCallback(async (view: ViewKey) => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, [view]: (error as Error).message || 'Beklenmeyen hata' }))
...
function deriveAssetsFromDetail(detail: MatchDetail | null): MatchAssets | null {
