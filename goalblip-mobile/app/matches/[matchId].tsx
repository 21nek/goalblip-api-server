...
export default function MatchDetailScreen() {
  ...
  const fetchDetail = useCallback(
    async (showLoading = true, retryCount = 0) => {
      if (!numericId) {
      ...
      const tryFetch = async (opts?: { date?: string; view?: 'today' | 'tomorrow' | 'manual' }) => {
        const response = await getOrFetchMatchDetail(numericId, opts);
        if (response) {
      ...
      try {
        ...
        const ok = await tryFetch(options);

        // If date/view-specific fetch did not return data, avoid grabbing a different date; wait for queue or retry
        if (!ok && options) {
          const pending = getPendingMatch(numericId, options.date);
          // Parametreli istek null döndüyse pending olabilir; farklı dosyaya düşmemek için bekleyip tekrar dene
          setLoading(false);
          setRefreshing(false);
          if (pending) {
            if (!retryTimeoutRef.current) {
              retryTimeoutRef.current = setTimeout(() => {
                ...
                fetchDetail(false, retryCount + 1);
              }, 20000);
            }
          } else {
            setError(t('match.detailNotFound'));
          }
          return;
        }
        ...
        // On error, attempt a paramless fallback once
        if (options) {
          try {
            console.log('[MatchDetail] Falling back to fetch without date/view after error');
            const ok = await tryFetch(undefined);
            if (ok) { return; }
          } catch (fallbackErr) {
            console.error('[MatchDetail] Fallback fetch failed:', fallbackErr);
          }
        }
  ...
  useEffect(() => {
    ...
    // Ýlk yüklemede cache'i göster, ama arka planda fresh data çek
    if (cachedDetail) {
      setDetail(cachedDetail);
      setLoading(false);
  ...
  }, [numericId, dateParam, viewParam]); // matchId, date veya view deðiþtiðinde yeniden fetch et
        ...
        {loading ? (
          ...
          <View style={getStyles().errorBox}>
            <EmptyState
              ...
              message={t('matchDetail.retryLaterMessage')}
              action={
                <View style={{ alignItems: 'center' }}>
                  <TouchableOpacity
                    style={getStyles().retryButton}
                  ...
                  <TouchableOpacity
                    style={[getStyles().retryButton, getStyles().reanalyzeButton]}
                    onPress={handleReanalyzeRequest}
                    disabled={isReanalyzeDisabled}
                    activeOpacity={0.7}
                  >
                    <Text style={getStyles().retryText}>
                      {reanalyzeLoading ? t('matchDetail.reanalyze.buttonLoading') : t('matchDetail.reanalyze.button')}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[getStyles().retryButton, getStyles().backButton]}
